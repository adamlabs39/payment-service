  ...
  try {
    ...
    if (patientUuidsToDelete.length > 0) {
      ...
      if (billUuidsToDelete.length > 0) {
        ...
        if (serviceBillUuidsToDelete.length > 0) {
          await queryInterface.bulkDelete(
            'bill_item',
            { service_bill_uuid: { [Op.in]: serviceBillUuidsToDelete } },
            { transaction }
          );
        }
        ...
        await queryInterface.bulkDelete(
          'payment_history',
          { bill_uuid: { [Op.in]: billUuidsToDelete } },
          { transaction }
        );
      }
      ...
      if (birthDetailUuidsToDelete.length > 0) {
        await queryInterface.bulkDelete(
          'birth_details',
          { uuid: { [Op.in]: birthDetailUuidsToDelete } },
          { transaction }
        );
      }
    }
    ...
    await queryInterface.bulkInsert(
      ...
      faskesList.map((faskes) => ({
        ...
        address_uuid: uuidv7(),
        phone: '021-1234567',
        email: `klinik@${faskes.code}.com`,
        website: `https://klinik-${faskes.code}.com`,
      ...
      vouchersToSeed.push(
        {
          uuid: uuidv7(),
          faskes_uuid: faskes.uuid,
          code: `SEEDER-OK-${faskes.code}`,
          name: 'Voucher Diskon 10 Persen',
          type: 'persentase',
          value: 10,
          qty: 100,
          status: true,
          start_date: moment().subtract(1, 'day').unix(),
          end_date: moment().add(1, 'month').unix(),
          created_at: moment().unix(),
          updated_at: moment().unix(),
        },
        {
          uuid: uuidv7(),
          faskes_uuid: faskes.uuid,
          code: `SEEDER-BESAR-${faskes.code}`,
          name: 'Voucher Potongan 300rb',
          type: 'potongan',
          value: 300000,
          qty: 5,
          status: true,
    ...
    const serviceTemplates = [
      {
        type: 'RJ',
        serviceName: 'Konsultasi Rawat Jalan',
      ...
      {
        type: 'RI',
        serviceName: 'Perawatan Rawat Inap Anak',
        with_insurance: true,
        items: [
      ...
      {
        type: 'IGD',
        serviceName: 'Tindakan Gawat Darurat',
        with_insurance: false,
        items: [
      ...
      {
        type: 'OTC',
        serviceName: 'Pembelian Obat Bebas',
        with_insurance: true,
        items: [
    ...
    for (const faskes of faskesList) {
      ...
      for (let i = 1; i <= 50; i++) {
        ...
        if (selectedService.type === 'RJ') {
          ...
          const tanggalPeriksa = scheduleBaseDate
            .hour(startHour)
            .minute(startMinute)
            .add(i * 15, 'minutes')
            .unix();
            ...
            [
              {
                ...
                patient_uuid: patientUuid,
                name: `Pasien Seed ${i} ${faskes.code}`,
                invoice_code: `INV-${faskes.code}-00${i}`,
                bill_code: `BILL-${faskes.code}-00${i}`,
                status: false,
                sub_total: subTotal,
                ppn: ppn,
                admin_fee: adminFee,
